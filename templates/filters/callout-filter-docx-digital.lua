--[[
Callout Filter for Constellize Book -- DOCX output, DIGITAL variant

Twin of callout-filter-docx-print.lua. The two files are a near-identical pair:
they share a PRIMITIVES region and a BODY region that are byte-identical, and
differ only in the VARIANT region in the middle, which draws the box chrome.
...
This variant mirrors the digital tcolorbox in templates/book-digital.latex:
`frame hidden` with a coloured left bar drawn by an overlay, a `!5!white` body
tint, and the title set in the accent colour inside the box.

Usage:
  pandoc ... -t docx --lua-filter=callout-filter-docx-digital.lua
  pandoc ... -t docx --lua-filter=callout-filter-docx-digital.lua \
...
-- VARIANT: digital
-- ----------------------------------------------------------------------------
-- THE ONLY REGION THAT DIFFERS FROM callout-filter-docx-print.lua.
--
-- Mirrors the digital tcolorbox (templates/book-digital.latex):
...
local VARIANT = {
  id = "digital",
  -- Screen readers can click the hyperlink, so the raw URL is not repeated.
...
--- Mirrors `frame hidden, boxrule=0pt, colback=<speaker>!10!white`.
